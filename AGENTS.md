...
- **ESLint Version**: Supports ESLint 9+ with flat configuration format (bundled `eslint`; ESLint 10 when peer plugins support it)
...
├── scripts/
│   └── verify-node-pins.cjs  # Asserts `.nvmrc` matches Dockerfile Node tag
├── eslint.config.js      # Repo-local ESLint (extends published config)
├── Dockerfile            # Pinned Node image (aligned with `.nvmrc`)
├── .nvmrc                # Pinned Node patch for local dev and CI
├── tsconfig.json         # Minimal TS project for `@typescript-eslint` type-aware rules in this repo
...
1. **Installation**: Install as dev dependency together with listed peer dependencies; **ESLint is a normal dependency** of this package (consumers do not add `eslint` separately).
...
- **Peer Dependencies**: Consumers install the ESLint plugins/parsers this config uses; **ESLint itself** is bundled as a dependency of `@dendavidov/eslint-config-react`.
...
- **Type Definitions**: `src/index.d.ts` (`Linter.Config[]`, `export =` for typings)
...
- `pnpm test`: Node pin verification, then Jest tests
- `pnpm run format`: Apply Prettier formatting
- `pnpm run format:check`: Check Prettier formatting
- `pnpm run lint`: ESLint (repo `eslint.config.js`)
- `pnpm run release`: Trigger semantic release
...
- **Dependencies**: Ship `eslint` as a runtime dependency; keep ESLint plugins as peer dependencies so consumers control plugin versions.
- **Version Compatibility**: Maintain compatibility with ESLint 9+ and Node.js 24.x (`engines.node`)
